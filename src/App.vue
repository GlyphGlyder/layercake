...
export default {
  ...
  methods: {
    ...
    buildPaths: function(paths) {
      ...
      for(var i = 0; i < paths.length; i ++) {
        ...
        for(var j = 0; j < pathCommands.length; j ++) {
          ...
          this.shapeSwitchCommand(shape, pathCommands[j]);
     ...
     */
    shapeSwitchCommand: function(shape, command) {
      ...
      switch(letter[1]) {
        case ("M"):
          ...
          if (points.length == 2) {
            shape.moveTo(
              parseFloat(points[0]) - this.originOffset.x,
              this.originOffset.y - parseFloat(points[1])
            );
          }
          break;
        case ("m"):
        ...
        case ("V"):
          ...
          if (points.length == 1) {
            shape.lineTo(
              ...
              this.originOffset.y - parseFloat(points[0])
            );
          }
          break;
        case ("v"):
        ...
        case ("A"):
          ...
          let rx = parseFloat(points[0]);
          let ry = parseFloat(points[1]);
          let xar = parseFloat(points[2]);
          let laf = parseInt(points[3]);
          let sf = parseInt(points[4]);
          let nx = parseFloat(points[5]) - this.originOffset.x;
          let ny = this.originOffset.y - parseFloat(points[6]);

          let x = shape.currentPoint.x;
          let y = shape.currentPoint.y;

          let x1 = Math.cos( xar ) * ( x - nx ) / 2 + Math.sin( xar ) * ( y - ny ) / 2;
          let y1 = - Math.sin( xar ) * ( x - nx ) / 2 + Math.cos( xar ) * ( y - ny ) / 2;

          // Fuck this shit
          let norm = Math.sqrt( ( rx * rx * ry * ry - rx * rx * y1 * y1 - ry * ry * x1 * x1 ) /
									 ( rx * rx * y1 * y1 + ry * ry * x1 * x1 ) );

          // This was causing weird bullshit.  Had to nuke it.
          //if ( laf === sf ) norm = - norm;

          let x2 = norm * rx * y1 / ry;
					let y2 = norm * -ry * x1 / rx;

          let cx = Math.cos( xar ) * x2 - Math.sin( xar ) * y2 + ( x + nx ) / 2;
				  let cy = Math.sin( xar ) * x2 + Math.cos( xar ) * y2 + ( y + ny ) / 2;

          let u = new THREE.Vector2( 1, 0 );
					let v = new THREE.Vector2( ( x1 - x2 ) / rx, ( y1 - y2 ) / ry );
          ...
          shape.absarc(cx, cy, rx, startAng, startAng + deltaAng, sf);
          break;
